...
#
# def register_page (request):
#     # post methodga teshiruv beramiza
#     if request.method == 'POST':
#         # username, password1, password2 ovolamiza
#         username = request.POST.get('username')
#         password1 = request.POST.get('password1')
#         password2 = request.POST.get('password2')
#
#         #Userdan objectimizzi filtratsiya qvolamiza agar 1 - username bazada mavjud bolsa error chiqadi 2- agar bolmasa yaratadi
#         if password1 == password2:
#             # 1
#             if User.objects.filter(username=username).exists():
#                 return render(request, template_name='register.html', context={'error': 'Username already exists'})
...
#
# def login_page (request):
#     if request.method == 'POST': # sitan post methoga teshiruv beramiz
#
#         # username bilan passworddi ovolamiz
#         username = request.POST.get('username')
#         password1 = request.POST.get('password1')
#         # password bilan username malumotimizi tasdiqlimiz
#         user = authenticate(username=username, password1=password1)
#
#         # 1 - agar userimiz bazada bolsa userimizi tasdiqlab home redirect qiladi 2 - agar bazada bolmasa login html render qiladi
#         if user is not None:
#             login(request, user)
#             return redirect('home')
#         else:
#             return render(request, template_name='login.html', context={'error': 'Invalid username'})
#
#     return render(request, template_name='login.html')
#
#
